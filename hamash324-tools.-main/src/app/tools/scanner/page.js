...
export default function WifiAnalyzer() {
  ...
  const analyzeWifi = (e) => {
    ...
    setReport({
      ...
      securityScore: `${score}%`,
      recommendation: score > 70 ? 'Current settings are secure and compliant.' : 'Recommended to change password and add special characters.'
    });
  };

  return (
    <main style={{ minHeight: '100vh', backgroundColor: '#090a0f', color: '#00e5ff', fontFamily: 'monospace', padding: '2rem', maxWidth: '850px', margin: '0 auto', direction: 'rtl', textAlign: 'right' }}>
      <nav style={{ marginBottom: '2rem' }}>
        <Link href="/" style={{ color: '#64748b', textDecoration: 'none' }}>← Back to Dashboard</Link>
      </nav>

      <h1 style={{ color: '#fff', fontSize: '1.6rem', marginBottom: '0.5rem' }}>Network & WiFi Security Scanner</h1>
      <p style={{ color: '#9ca3af', lineHeight: '1.6', marginBottom: '2rem' }}>
        Advanced tool for checking network security, password strength, and vulnerability assessment.
      </p>
